...
import { CaixasTab } from "@/components/cartoes/caixas-tab"
import { getCaixas } from "./actions"

export const metadata: Metadata = { title: "Cartões e Caixas" }
...
export default async function CartoesPage() {
  ...
  const [{ data: cartoes }, { data: empresas }, { data: usuarios }, caixas] =
    await Promise.all([
      supabase
        .from("cartoes")
        .select("id, nome, banco, empresa_id, usuario_id, dia_vencimento, dia_fechamento, ativo, created_at")
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-white/55">
          Cartões da agência e contas bancárias onde os recebimentos são depositados.
        </p>
      </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-white/[0.06] hover:bg-transparent">
                    ...
                    <TableHead className="text-right text-white/55">Fechamento</TableHead>
                    <TableHead className="text-right text-white/55">Vencimento</TableHead>
                ...
                <TableBody>
                  {!cartoes || cartoes.length === 0 ? (
                    <TableRow className="border-white/[0.06] hover:bg-transparent">
                      <TableCell colSpan={8} className="h-24 text-center text-sm text-white/45">
                        Nenhum cartão cadastrado.
                      </TableCell>
                    </TableRow>
                  ) : (
                    cartoes.map((c) => (
                      <TableRow key={c.id} className="border-white/[0.06] hover:bg-white/[0.025]">
                        <TableCell className="font-medium text-white">{c.nome}</TableCell>
                        <TableCell className="text-sm text-white/75">{c.banco || "—"}</TableCell>
                        <TableCell className="text-sm text-white/75">{empresasMap.get(c.empresa_id) ?? "—"}</TableCell>
                        <TableCell className="text-sm text-white/75">{usuariosMap.get(c.usuario_id) ?? "—"}</TableCell>
                        ...
                        <TableCell className="text-right">
                          <CartaoRowActions
                            cartao={{
                              ...
                              empresa_id: c.empresa_id, usuario_id: c.usuario_id,
                              dia_vencimento: c.dia_vencimento, dia_fechamento: c.dia_fechamento,
                              ativo: c.ativo,
                            }}
                  ...
                  <div key={c.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        {c.banco && <p className="mt-0.5 text-xs text-white/55">{c.banco}</p>}
                      </div>
                      {c.ativo ? (
