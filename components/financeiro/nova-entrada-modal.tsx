...
import { Plus, Paperclip, X, FileText, Loader2 } from "lucide-react"
...
export function NovaEntradaButton({
  categorias,
  caixas,
  cartoes,
}: {
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 rounded-lg border border-nexus-bright/25 bg-nexus-bright/[0.08] px-3 py-2 text-sm font-medium text-nexus-bright transition-colors hover:border-nexus-bright/50 hover:bg-nexus-bright/15"
      >
...
export function NovaEntradaModal({ open, onClose, categorias, caixas, cartoes }: Props) {
  const hoje = hojeIso()
  ...
  // Arquivos selecionados antes da criação
  const [stagedFiles, setStagedFiles] = useState<File[]>([])
  // Anexos confirmados após upload
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (criadoId) {
      // Lançamento já criado → upload imediato
      uploadFile(criadoId, file)
    } else {
      // Ainda no formulário → fila local
  ...
  async function uploadFile(id: string, file: File) {
    ...
    fd.append("file", file)
    const r = await uploadAnexoLancamento("receber", id, fd)
    setUploading(false)
  ...
  function submit() {
    ...
    startTransition(async () => {
      const r = await criarLancamentoReceber({
        descricao,
      ...
      // Enviar arquivos que estavam em fila
      if (stagedFiles.length > 0) {
        setUploading(true)
        for (const file of stagedFiles) {
          const fd = new FormData()
          fd.append("file", file)
          const ur = await uploadAnexoLancamento("receber", novoId, fd)
          if (ur.ok) setAnexos((prev) => [...prev, ur.data!])
        }
        setStagedFiles([])
        setUploading(false)
      }
  ...
  const limiteAnexos = 2
  const atingiuLimite = totalArquivos >= limiteAnexos
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Plus className="h-4 w-4 text-nexus-bright" />
            Nova Conta a Receber
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-1">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="ex: Estorno referente ao pedido #123"
          ...
          <div className="space-y-2 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
            ...
            {totalArquivos === 0 ? (
              <p className="text-[11px] text-white/30">Nenhum arquivo selecionado.</p>
            ) : (
              <div className="space-y-1.5">
                {/* Arquivos em fila (antes de criar) */}
                {stagedFiles.map((f, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-md border border-white/[0.04] bg-white/[0.02] px-2.5 py-1.5">
                    <FileText className="h-3.5 w-3.5 shrink-0 text-white/30" />
                    <span className="flex-1 truncate text-xs text-white/50">{f.name}</span>
                    <span className="text-[10px] text-white/25">pendente</span>
                    <button
                      type="button"
