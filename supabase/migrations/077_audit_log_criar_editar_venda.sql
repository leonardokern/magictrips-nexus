-- =============================================================================
-- 077 — audit_logs: registra criação e edição de vendas
-- =============================================================================
-- Bug encontrado: `criar_venda_completa` só gravava em `audit_logs` quando o
-- caminho de auto-aprovação acionava (usuário com `vendas.aprovar`). Quando um
-- agente sem permissão de aprovar criava uma venda (status pendente_validacao),
-- nenhum registro era escrito — a venda nascia sem rastro. Sintoma: Nicolli
-- Saikoski criou MT-0019 no sábado 20/jun e o modal de logs dela só mostra
-- login + alterar_senha.
--
-- `editar_venda_completa` também nunca registrava nada — edições silenciosas
-- somem do histórico, e o atalho `p_aprovar=true` aprovava sem audit.
--
-- Esta migration redefine as duas funções acrescentando os INSERTs de audit
-- nos pontos certos. O resto do corpo é cópia idêntica das versões em
-- produção (puxadas via `pg_get_functiondef`), preservando as colunas mais
-- recentes em `cobranca_cliente_itens` (plataforma, parcelas_detalhe,
-- comprovante_*) que evoluíram em migrations posteriores.
-- =============================================================================
...
BEGIN
  ...
  -- Audit log de criação (SEMPRE, independente de auto-aprovação).
  -- v_uid é quem executou a ação; v_usuario_id é o agente vinculado à venda
  -- (pode ser outro quando admin/gerente cadastra em nome do agente).
  INSERT INTO audit_logs (
    usuario_id, empresa_id, entidade, entidade_id, acao, dados_depois
  ) VALUES (
    v_uid, v_empresa_id, 'venda', v_venda_id, 'criar',
    jsonb_build_object(
...
-- Espelha a versão atual no banco (com colunas plataforma/comprovante em
-- cobranca_cliente_itens) e acrescenta INSERT em audit_logs ao final.
...
BEGIN
  ...
  v_status_anterior := v_venda.status;
  v_identificador   := v_venda.identificador;
  ...
  FOR v_produto IN SELECT * FROM jsonb_array_elements(v_produtos) LOOP
    ...
    INSERT INTO venda_produtos (
      ...
      valores_extras, tipo_comissao, valor_venda, valor_custo, rav, rav_extra_cliente, rav_extra_fornecedor,
      ...
      nullif(v_produto->>'rav', '')::numeric, COALESCE((v_produto->>'rav_extra_cliente')::numeric, 0), COALESCE((v_produto->>'rav_extra_fornecedor')::numeric, 0),
  ...
  -- Audit log de edição — sempre executa, mesmo quando o atalho de aprovação
  -- dispara também. Resultam 2 logs nesse caso: 'editar' + 'aprovar'.
  INSERT INTO audit_logs (
    usuario_id, empresa_id, entidade, entidade_id, acao, dados_depois
  ) VALUES (
    v_uid, v_empresa_id, 'venda', p_venda_id, 'editar',
    jsonb_build_object(
      'identificador',   v_identificador,
  ...
  IF v_aprovou THEN
    INSERT INTO audit_logs (
      ...
      v_uid, v_empresa_id, 'venda', p_venda_id, 'aprovar',
      jsonb_build_object(
        'identificador',   v_identificador,
        'status_anterior', v_status_anterior,
        'status_novo',     'aprovado',
        'via',             'editar_venda_completa'
      )
    );
  END IF;
END;
