-- =============================================================================
-- 047 — Data de emissão por produto
-- =============================================================================
-- Cada venda_produto recebe uma data própria de emissão (diferente da data
-- da venda). Para conferência operacional — não tem efeito em cálculos.
--
-- Vendas existentes: backfill copiando vendas.data_venda para evitar NULLs
-- na UI.
-- =============================================================================
...
-- Backfill: vendas existentes herdam data_venda
UPDATE venda_produtos vp
SET data_emissao = v.data_venda
FROM vendas v
WHERE vp.venda_id = v.id
  AND vp.data_emissao IS NULL;

-- ── RPC criar_venda_completa: incluir data_emissao no INSERT ─────────────────
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO venda_produtos (
      ...
      v_produto->>'destino',
      -- Backfill: se data_emissao não vier no payload, usa data_venda
      COALESCE(nullif(v_produto->>'data_emissao', '')::date, v_data_venda),
      ...
      COALESCE(nullif(v_produto->>'pgto_modo', ''), 'comissionado'),
