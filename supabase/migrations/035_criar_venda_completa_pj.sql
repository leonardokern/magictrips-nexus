-- =============================================================================
-- 035 — RPC `criar_venda_completa` aceita cliente_novo PJ
-- =============================================================================
-- O bloco de criação inline de cliente (quando cliente_id é NULL) agora
-- ramifica em `tipo_pessoa`:
--   * fisica (padrão) → exige nome + cpf; insert com cpf
--   * juridica        → exige razão social + cnpj; insert com cnpj/PJ fields
-- Lookup de duplicado também ramifica (cpf ou cnpj conforme o tipo).
-- Demais blocos da RPC ficam intactos.
-- =============================================================================
...
DECLARE
  ...
  v_tp             text;  -- tipo_pessoa do cliente novo
BEGIN
  ...
  IF v_cliente_id IS NULL THEN
    ...
    IF v_tp = 'fisica' THEN
      IF v_cliente_novo->>'nome' IS NULL OR v_cliente_novo->>'cpf' IS NULL THEN
        RAISE EXCEPTION 'Cliente novo PF precisa de nome e CPF.' USING ERRCODE = '22023';
      END IF;
      ...
      IF v_cliente_id IS NULL THEN
        INSERT INTO clientes (
          empresa_id, tipo_pessoa, nome, email, telefone, cpf, data_nascimento,
    ...
    ELSIF v_tp = 'juridica' THEN
      IF v_cliente_novo->>'razao_social' IS NULL OR v_cliente_novo->>'cnpj' IS NULL THEN
        RAISE EXCEPTION 'Cliente novo PJ precisa de razão social e CNPJ.' USING ERRCODE = '22023';
      END IF;
      ...
      IF v_cliente_id IS NULL THEN
        INSERT INTO clientes (
          empresa_id, tipo_pessoa, nome, razao_social, nome_fantasia, cnpj,
          responsavel, email, telefone, tipo, dia_faturamento, status
        ) VALUES (
          v_empresa_id, 'juridica',
          -- `nome` na tabela é NOT NULL — usa fantasia (fallback razão)
          COALESCE(
          ...
          nullif(v_cliente_novo->>'nome_fantasia', ''),
          v_cliente_novo->>'cnpj',
          nullif(v_cliente_novo->>'responsavel', ''),
          COALESCE(v_cliente_novo->>'email', ''),
          COALESCE(v_cliente_novo->>'telefone', ''),
          COALESCE(v_cliente_novo->>'tipo', 'regular'),
          nullif(v_cliente_novo->>'dia_faturamento', '')::int,
